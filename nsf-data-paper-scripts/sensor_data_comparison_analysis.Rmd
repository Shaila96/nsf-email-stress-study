...
t_test_hr <- function(df, plot_title, file_name) {
  ################################
  #####  CHANGE THE CODE!!!  #####
  ################################
  df <- df %>% na.omit
  ...
  t_test <- NA
  if (nrow(df) < 30) {
    t_test <- wilcox.test(df$HR, df$N.HR, data=df, paired=T)
  } else {
    t_test <- t.test(df$HR, df$N.HR, data=df, paired=T)
  }
...
t_test_hr(mean_hr_df, 'HR Boxplot - all sessions', 'hr-boxplot-all-sessions')
```
...
for (session_name in new_session_list) {
  if (session_name=='SC.RB') {
    ...
    t_test_hr(mean_hr_diff_df, 
              paste0('HR Boxplot - ', session_name), 
              paste0('hr-boxplot-', session_name))
  }
}
