...
get_scatter_plot <- function(df, x_col, y_col, x_label, y_label, title, transformed=F) {
  cor_test <- cor.test(df[[x_col]], df[[y_col]], method = "pearson")
  subj_no <- df %>% distinct(Subject) %>% summarize(n = n())
  
  # annot_label <- c(paste("n =", subj_no),
  #                   paste("p =", cor_test$p.value),
  #                   paste("R =", cor_test$estimate))
  annot_label <- paste("n =", subj_no, 
                       ", p =", specify_decimal(cor_test$p.value, 2), 
  ...
  plot_path <- file.path(plots_dir, replace_space(title))
...
generate_hr_plot <- function() {
  new_mean_diff_df <- new_mean_diff_df[, hr_col_list] %>% na.omit()
  get_scatter_plot(new_mean_diff_df, 
                   'HR', 
                   'N.HR', 
                   figure_out_labels('HR'), 
