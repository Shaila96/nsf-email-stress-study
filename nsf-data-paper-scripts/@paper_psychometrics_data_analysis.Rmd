...
draw_group_bar_chart_nasa_tlx <- function() {
  ...
  for (ques_sub_type in ques_sub_type_levels) {
    ...
    nasa_bar_plot <- get_nasa_bar_plot_df(freq_df) %>%
      ...
      geom_text(aes(label=paste0('n = ', Frequency)), vjust=-0.2, size=6, fontface ='italic') + ## USE BQUOTE TO MAKE ITALIC
      # annotate(aes(label=paste0('n = ', Frequency)), vjust=-0.2, size=6, fontface ='italic') +
      theme_bw() +
