...
get_proportion_test_stress_ques <- function() {
  stress_ques_df <- nasa_tlx_freq_df[nasa_tlx_freq_df$Question=='Stress_Question', ]
  comparison_option <- 4
  alternative_val <- 'greater'
    
  
  total_response <- nrow(stress_ques_df)
  agree_df <- stress_ques_df %>%
    filter(Answer > comparison_option) %>% 
    summarize(agree_freq = sum(Frequency))
  disagree_df <- stress_ques_df %>% 
    filter(Answer <= comparison_option) %>% 
    summarize(disagree_freq = sum(Frequency))
  ...
  # generate_nasa_bar_plot(freq_df, nasa_ques)
  plot <- get_nasa_bar_plot_df(freq_df) %>%
    ggplot(aes(x=Options, y=Frequency)) +
    geom_bar(stat='identity') +
    ggtitle(replace_underscore('Stress_Question Prop Test')) +
    scale_x_discrete(labels=c('agree_freq' = 'Agreement', 'disagree_freq' = 'Neutral/Disagreement')) +
    scale_y_discrete(expand=expand_scale(mult=c(0, 0.1), add=c(0, 1))) +
    geom_text(aes(label=paste0('n = ', Frequency)), vjust=-0.2, size=4) +
    theme_bw() + 
    theme(text=element_text(size=16),
          plot.title = element_text(hjust = 0.5))
...
draw_pss_plot <- function() {
  ...
  personality_plot_list[[length(personality_plot_list)+1]] <<- hist_plot

  
  # print(hist_plot)
  # save_plot(replace_dot_space(title), hist_plot, width=width, height=height)
...
draw_bfi_plot <- function() {
  ...
  for (ques_sub_type in ques_sub_type_levels) {
    ...
    # gg <- arrangeGrob(hist_plot, top = textGrob(toupper('A'),
    #                                     x = unit(0, "npc"),
    #                                     y = unit(1, "npc"),
    #                                     vjust = 2, 
    #                                     hjust = -3,
...
combine_psychometric_plots <- function() {
  right_side_plots <- plot_grid(plotlist=personality_plot_list,
                               labels=c('B', 'C', 'D'),
                               cols=1,
                               align='h',
                               rel_heights=c(2, 1, 1))
  # save_plot('personality_plots', right_side_plot)
  # final_plot <- plot_grid(right_side_plot, bfi_grid_plot, labels = c('A', ''), ncol = 2, rel_width = c(1, 1))
