...
ets_score_file_name <- 'essay_score_master_file.csv'
...
read_data <- function() {
  ...
  ets_score_df <<- read_csv(file.path(performance_data_dir, ets_score_file_name))[, c(1:6)]
...
extract_ets_score <- function() {
  colnames(ets_score_df) <- c('Subject', 'CriterionScore', 'GrammarErrors',
                              'UsageErrors', 'MechanicErrors', 'StyleErrors')
  ...
  new_ets_score_df <- extract_criterion_score(ets_score_df) %>%
    separate(Subject, c("Subject", "ConditionETS", 'Session'))
  # print_msg(str(new_ets_score_df))

  convert_to_csv(new_ets_score_df, file.path(performance_data_dir, 'ets_score_final.csv'))
}
``` 
