...
draw_essay_email_stacked_bar_plots <- function() {
  stacked_plot <- writing_time_dt_df %>% 
    ggplot(aes(fill=Writing, y=Time, x=Subject)) + 
...
extract_ets_score <- function() {
  ...
  ets_score_df <<- ets_score_df %>%
    ...
    mutate(Session = recode(Session, 'WB' = 'Single Task', 'DT' = 'Dual Task')) 
...
draw_ets_regression_plot <- function() {
  ...
  cor_test <- cor.test(ets_bio_df$Writing_Proficiency, ets_bio_df[['Criterion Score']], method = 'spearman')
  print_msg(cor_test)
  
  plot <- ets_bio_df %>%
    ggplot(aes(Writing_Proficiency, ets_bio_df[['Criterion Score']])) +
    geom_point(size = 3) +
    geom_smooth(method = 'lm')
  print(plot)
}
