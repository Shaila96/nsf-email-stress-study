  ...
  for(subj in subj_list) {
    ...
    for (session in session_list) {
      ...
      hr_plot <- df %>%
        ...
        geom_line(aes(y=df[[col_names[1]]]), color='coral') +
        geom_line(aes(y=df[[col_names[2]]]), color='dodgerblue') +
        ggtitle(paste0(subj, ': ', get_session_name(session))) +
        xlab('Time [s]') +
        ylab(figure_out_labels(col_names[1])) +
        geom_hline(yintercept=c(get_mean(session, col_names[1]), get_mean(session, col_names[2])), 
                   linetype="dashed", 
                   size=1.2,
      ...
      col_name <- 'N.EDA'
      eda_plot <- df %>%
        ggplot(aes(TimeElapsed, df[[col_name]])) +
        geom_line(color='mediumseagreen') +
        ggtitle(paste0(subj, ': ', get_session_name(session))) +
        xlab('Time [s]') +
        ylab(figure_out_labels(col_name)) +
        geom_hline(yintercept=get_mean(session, col_name), 
                   linetype="dashed", 
                   size=1.2,
                   color="darkgreen") +
        theme(plot.title = element_text(hjust = 0.5, face = "bold"))
