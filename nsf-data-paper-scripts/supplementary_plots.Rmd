...
filtered_data_file_name <- 'full_df_filtered_supplementary.csv'
...
draw_plots <- function() {
  ...
  for(subj in subj_list) {
    ...
    for (session in session_list) {
      ...
      if(nrow(eda_df)>0) {
        eda_y_lim <- if(max(eda_df[[eda_col_name]]) < 5) 5 else max(eda_df[[eda_col_name]])
        ...
        eda_plot <- eda_df %>%
          ...
          ylim(0, eda_y_lim) +
          geom_hline(yintercept=get_session_mean(session, eda_col_name),
                     size=1.2,
                     color="darkgreen") +
          theme(plot.title=element_text(hjust = 0.5, face = "bold"),
                legend.title=element_blank(),
