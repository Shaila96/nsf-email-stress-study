...
print_plot <- function(plot, title, y_lab='Frequency',  x_axis_text_size=16, angle=0) {
  plot <- plot +
...
get_x_limit <- function(erq_type) {
  if (erq_type=='Cognitive Reappraisal') {
    return(c(6, 42))
  } else if (erq_type=='Expressive Suppression') {
    return(c(4, 28))
  }
}
...
draw_hist_plot <- function(df, title) {
  ...
  # print_msg(nlevels(ques_sub_type_levels))
  # print_msg(length(ques_sub_type_levels))
  ...
  for (ques_sub_type in ques_sub_type_levels) {
    ...
    hist_plot <- ggplot(data=temp_df, aes(x=Value)) + 
      geom_bar() +
      facet_grid(. ~ title) +
      coord_cartesian(xlim = get_x_limit(ques_sub_type)) +
      xlab(x_lab) +
      ylab('') +   ##'Frequency'
      theme_bw() +
      theme(legend.position = 'none',
  ...
  grid_plot <- grid.arrange(do.call('grid.arrange', c(plot_list, ncol=1)), 
                            top=textGrob(paste(title, '- Histogram'), gp=gpar(fontsize=22, font=1)))
  save_plot(replace_dot_space(paste(title, '- Histogram')), grid_plot)
}
