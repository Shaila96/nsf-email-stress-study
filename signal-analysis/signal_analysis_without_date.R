...
split_session <- function() {
  ...
  #   write(paste('Current Session: ', session_level), file=log.file, append=TRUE)
  # 
  #   start_index <- 1
  #   while(1) {
  ...
  #       write(paste(
  #         'Start Time: ', floor(session_level_df[start_index, 1]), '      ',
  #         'End Time: ', floor(session_level_df[start_index + nrow(consecutive_df) - 1, 1])),
  #         file=log.file,
  #         append=TRUE)
  # 
  ...
  # ###################################################################################################################################
  # 
  # # pp_with_session_df <- pp_with_session_df[c(1:400),]
  # 
  # # downsample_using_mean_with_date
  ...
  sim2_file_name <- getMatchedFileNames(getwd(), sim2_file_pattern)
  sim2_df <- read_csv(file.path(getwd(), sim2_file_name))
  # sim2_df$NR_Perspiration <- remove_noise(sim2_df$Perspiration)
  # convert_to_csv(sim2_df, file.path(getwd(), paste0(substr(sim2_file_name, 1, nchar(sim2_file_name)-4), '_nr_pp.csv')))
  # 
  # downsampled_pp_df <- downsample_using_mean(sim2_df, c('Perspiration', 'NR_Perspiration'))
  # convert_to_csv(downsampled_pp_df, file.path(getwd(), paste0(substr(sim2_file_name, 1, nchar(sim2_file_name)-4), '_downsampled.csv')))
  
  
  downsampled_pp_df <- downsample_using_mean(sim2_df, c('Perspiration'))
  convert_to_csv(downsampled_pp_df, file.path(getwd(), paste0(substr(sim2_file_name, 1, nchar(sim2_file_name)-4), '_downsampled_2.csv')))
  
  downsampled_pp_df$NR_Perspiration <- remove_noise(downsampled_pp_df$Perspiration)
  convert_to_csv(downsampled_pp_df, file.path(getwd(), paste0(substr(sim2_file_name, 1, nchar(sim2_file_name)-4), '_nr_pp_2.csv')))
